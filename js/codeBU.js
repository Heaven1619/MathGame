...
function hardness() {
  ...
  '<label for="rad_hard">ساده ( امتیاز کم )</label> <span class="glyphicon glyphicon-lock"> </span>'+
...
if(score>=540 && score<600)//Level 10 - missed plus
{
 ...
 var val_other4 = 1;
  ...
  var which_answ_rand = Math.random(), // find a random answer block 1 or 2 or 3 or 4
    ...
    if(level_check != 2 && level_check != 9) //cuz in level2,9 in answ li true or false is writen
    {
    var li = $('ul li');
...
function result() //User Answer Check
{
  ...
  if(selected == sum)
  {
    ...
    switch(level_check)
    {
      ...
      case 6 :
      case 7 :
      case 8 : sec+=(reward_Time+1); score+=(reward_Score+3); break;
    }
  }
...
function show_best_records (period) {
    ...
    if(period == 'total')
    {
      ...
      $.post('http://heavenr.skyf.ir/Math-UI/php/get_records.php',{'username':username},function(data){
    ...
    if(period == 'today')
    {
      $.post('http://heavenr.skyf.ir/Math-UI/php/get_records_today.php',{'username':username},function(data){
        $('#tbody').html(data);
      });
    }
}
